...
import { Eye, EyeOff, CheckCircle2, Loader2, Moon, Sun, Bug } from "lucide-react";
...
export function SettingsClient({ userId, appEnv }: SettingsClientProps) {
  ...
  // Dev mode: global API key
  const [devKey, setDevKey] = useState("");
  const [devKeySet, setDevKeySet] = useState(false);
  ...
  useEffect(() => {
    if (isDev) {
      // Check if a dev key is already set
      fetch("/api/dev-key")
        .then(r => r.json())
        .then(data => setDevKeySet(data.hasKey))
        .catch(() => {})
  ...
  const handleSave = async () => {
    ...
    try {
      if (isDev) {
        // Dev mode: save global API key (no encryption)
        const res = await fetch("/api/dev-key", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ key: devKey || null }),
        });
        const data = await res.json();
        if (data.error) throw new Error(data.error);
        setDevKeySet(data.hasKey);
        toast.success(data.hasKey ? "全局 API Key 已设置" : "全局 API Key 已清除");
        setDevKey("");
      } else {
        // Test/Prod mode: save encrypted per-user keys to DB
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          {isDev ? "🔬 开发模式 · API Key 为全局共享，不加密" : "管理你的训练偏好和 API 配置"}
        </p>
      </div>
        ...
        <Card className="border-amber-200 dark:border-amber-800">
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-muted-foreground">
              开发模式下，API Key 作为全局变量使用，不加密，不保存到数据库。用于快速调试 AI 功能。
              {devKeySet && <span className="ml-2 text-green-600 font-medium">✅ 已设置</span>}
            </p>
            <div className="space-y-2">
              <Label>全局 API Key</Label>
            ...
            <div className="flex gap-2">
              <Button onClick={handleSave} disabled={saving || (!devKey && !devKeySet)}>
                {saving ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <CheckCircle2 className="h-4 w-4 mr-1" />}
                {devKey ? "设置" : devKeySet ? "清除" : "设置"}
              </Button>
            </div>
          </CardContent>
        </Card>
      ...
      {/* Test/Prod Mode: Per-user AI Configuration */}
      {!isDev && (
        <Card>
          <CardHeader>
      ...
      <Card>
        ...
        <CardContent className="text-sm text-muted-foreground space-y-2">
          ...
          <p>运行环境: {appEnv === "dev" ? "🔬 开发版" : appEnv === "test" ? "🧪 测试版" : "🚀 生产版"}</p>
